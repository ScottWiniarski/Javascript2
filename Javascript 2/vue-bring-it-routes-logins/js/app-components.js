    ...
    created(){
        ...
        switch(this.collection){
            ...
            case 'user':
                ...
                query = db.collection('potlucks')
                    .where('createBy.uid', '==', this.user.uid);
                break;
            default:
                query = db.collection('potlucks')
                    .where('datetime', '>', new Date())
        ...
        query.onSnapshot(snapshot => {
            ...
            snapshot.forEach(doc => {
                this.potlucks.push({id: doc.id, ...doc.data()});
            });
        });
    },
    ...
    created(){
        db.collection('potlucks')
            ...
            .onSnapshot(snapshot => {
                this.guests = [];
                snapshot.forEach(doc => {
                    this.guests.push({id: doc.id, ...doc.data()});
                });
            });
        ...
        db.collection('potlucks')
            ...
            .onSnapshot(snapshot => {
                this.needs = [];
                snapshot.forEach(doc => {
                    this.needs.push({id: doc.id, ...doc.data()});
                });
            })
...
Vue.component('navigation', {
    ...
    methods: {
        login(){
            ...
            // can replace line below with the auth.signIn featured below, no significant difference.
            //firebase.auth()
